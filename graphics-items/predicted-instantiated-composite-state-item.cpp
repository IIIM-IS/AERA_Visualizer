...
#include "predicted-instantiated-composite-state-item.hpp"
...
PredictedInstantiatedCompositeStateItem::PredictedInstantiatedCompositeStateItem(
  ...
  : AeraGraphicsItem(newInstantiatedCompositeStateEvent,
    ...
    parent,
    "Comp. State " + 
    makeHtmlLink(newInstantiatedCompositeStateEvent->object_->get_reference(0)->get_reference(0)->get_reference(0)->get_reference(0), replicodeObjects) +
    " " + RightDoubleArrowHtml + "<br>&nbsp;&nbsp;Pred. Instantiated Comp. State"),
...
void PredictedInstantiatedCompositeStateItem::setFactIcstHtml()
{
  auto factPredFactIcst = (_Fact*)newInstantiatedCompositeStateEvent_->object_;
  auto predFactIcst = ((_Fact*)newInstantiatedCompositeStateEvent_->object_)->get_pred();
  auto factIcst = (_Fact*)predFactIcst->get_reference(0);
  ...
  string factIcstSource = regex_replace(
    replicodeObjects_.getSourceCode(factIcst), confidenceAndSaliencyRegex, ")");
...
void PredictedInstantiatedCompositeStateItem::setBoundCstAndMembersHtml()
{
  auto factPredFactIcst = (_Fact*)newInstantiatedCompositeStateEvent_->object_;
  auto predFactIcst = ((_Fact*)newInstantiatedCompositeStateEvent_->object_)->get_pred();
  auto factIcst = (_Fact*)predFactIcst->get_reference(0);
  ...
  InstantiatedCompositeStateItem::getIcstOrImdlValues(icstSource.c_str(), templateValues, exposedValues);
  int iAfterVariable;
  int iBeforeVariable;
...
QString PredictedInstantiatedCompositeStateItem::makeHtml()
{
  QString html = "";
  if (showState_ == HIDE_ICST) {
    ...
    for (int i = 0; i < newInstantiatedCompositeStateEvent_->inputs_.size(); ++i) {
      ...
      auto input = (_Fact*)newInstantiatedCompositeStateEvent_->inputs_[i];
      if (input->get_pred())
        html += "predicted ";
      html += makeHtmlLink(input);
    }

    html += " matched composite state " + makeHtmlLink(cst) + ", which made predicted instantiated composite state <b>" +
      replicodeObjects_.getLabel(factIcst).c_str() + "</b>:<br>";
    html += "(fact (pred " + factIcstHtml_ + "))<br><br>" + boundCstHtml_;
  }

  return html;
}

void PredictedInstantiatedCompositeStateItem::textItemLinkActivated(const QString& link)
{
  if (link == "#hide-icst") {
